...
function Calendar({ onSelectRange }) {
    ...
    const handleDateClick = (date) => {
        if (!selectedStartDate) {
            ...
            if (date < selectedStartDate) {
                ...
                onSelectRange(formatDate(selectedStartDate), formatDate(date)); // 선택된 범위를 상위 컴포넌트로 전달
            }
        } else {
    ...
    const formatDate = (dateString) => {
        ...
        const day = String(date.getDate()).padStart(2, "0"); // 일은 1부터 시작
    ...
    const renderCalendar = () => {
        ...
        return weeks.map((week, weekIndex) => (
            ...
            ${
                date && date >= firstDayOfMonth && date <= lastDayOfMonth
                    ? "current-month"
                    : "other-month"
            } 
