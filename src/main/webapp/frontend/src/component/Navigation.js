...
import './css/Navigation.css'; // Navigation 스타일링을 위한 CSS 파일
import ChatIcon from './img/Chat.png'; // 내장된 이미지 파일 import
import AlaramIcon from './img/Alarm.png'; // 내장된 이미지 파일 import
import WriteModal from './WriteModal.js'; // 글쓰기 모달 컴포넌트 import
    ...
    return (
      <nav className="navigation">
        ...
        <ul className="nav-links">
          ...
          <li>
            <Link to="/about" className="nav-link">여행 커뮤니티</Link>
          </li>
          <li>
            <Link to="/services" className="nav-link">같이 가요!</Link>
          </li>
        </ul>
        ...
        <div className="icons-container">
          ...
          {/* 글쓰기 버튼 */}
          <div className="write-button" onClick={handleWriteModalToggle}>글쓰기</div>
        </div>
        ...
        {/* 글쓰기 모달 */}
        {writeModalOpen && (
          <WriteModal onClose={handleWriteModalToggle} />
        )}
      </nav>
    );
}
